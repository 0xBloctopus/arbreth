...
#[derive(Debug, Clone)]
pub struct UnsignedUserTxBuilder {
    pub from: Address,
    pub gas_limit: u64,
...
impl UnsignedUserTxBuilder {
    pub fn encode_body(&self) -> Vec<u8> {
        let mut out = Vec::with_capacity(1 + 32 * 5 + self.data.len());
        out.push(kinds::KIND_UNSIGNED_USER_TX);
        out.extend_from_slice(&encode_uint256(U256::from(self.gas_limit)));
        out.extend_from_slice(&encode_uint256(self.max_fee_per_gas));
        out.extend_from_slice(&encode_uint256(U256::from(self.nonce)));
...
impl MessageBuilder for UnsignedUserTxBuilder {
...
mod tests {
    ...
    fn body_layout_offsets() {
        let payload = Bytes::from(vec![0xab, 0xcd]);
        let to = address!("00000000000000000000000000000000000000bb");
        let s = sample(payload.clone(), to);
        let msg = s.build().unwrap();
        let body = decode_body(&msg);
        assert_eq!(body[0], kinds::KIND_UNSIGNED_USER_TX);
        assert_eq!(U256::from_be_slice(&body[1..33]), U256::from(s.gas_limit));
        assert_eq!(U256::from_be_slice(&body[33..65]), s.max_fee_per_gas);
        assert_eq!(U256::from_be_slice(&body[65..97]), U256::from(s.nonce));
        assert_eq!(Address::from_slice(&body[97 + 12..129]), to);
        assert_eq!(U256::from_be_slice(&body[129..161]), s.value);
        assert_eq!(&body[161..], payload.as_ref());
    }

    #[test]
    fn parses_into_unsigned_user_tx() {
        let to = address!("00000000000000000000000000000000000000bb");
        let s = sample(Bytes::from(vec![0x55, 0x66]), to);
        let msg = s.build().unwrap();
        ...
        match &txs[0] {
            ParsedTransaction::UnsignedUserTx {
                ...
                assert_eq!(*nonce, s.nonce);
                assert_eq!(data.as_slice(), &[0x55, 0x66]);
            }
    ...
    #[test]
    fn create_uses_zero_to() {
        let s = sample(Bytes::from(vec![0xfe]), Address::ZERO);
        let msg = s.build().unwrap();
        let parsed = round_trip(&msg);
        let txs = parse_l2_transactions(
            parsed.header.kind,
        ...
        .unwrap();
        match &txs[0] {
            ParsedTransaction::UnsignedUserTx { to, .. } => assert!(to.is_none()),
            other => panic!("unexpected parsed kind: {other:?}"),
        }
    }
