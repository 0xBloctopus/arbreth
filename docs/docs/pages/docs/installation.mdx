...
Arbitrum Reth will start syncing Arbitrum Sepolia. The Nitro consensus node waits for Arbitrum Reth to pass its healthcheck before starting.
...
Data is persisted in Docker named volumes (`arbitrum-reth-data`, `nitro-data`) and survives restarts.
